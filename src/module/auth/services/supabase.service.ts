...
export class SupabaseService {
   ...
   * O usuário é criado com email já confirmado e o Supabase envia email de boas-vindas automaticamente
   */
  async createUser(
    email: string,
    password: string,
    ...
    try {
      ...
      // Cria o usuário com email JÁ CONFIRMADO
      const { data: userData, error: createError } =
        await this.supabaseAdmin.auth.admin.createUser({
          email,
          password,
          email_confirm: true, // true = email já confirmado, não precisa validar
          user_metadata: {
      ...
      this.logger.log(
        `Usuário criado com email confirmado para: ${email} (ID: ${userData.user.id})`,
      );
      this.logger.log(
        `O Supabase enviará automaticamente o email de boas-vindas via SMTP configurado`,
      );
